...
interface Empresa {
  id: number;
  data_hora: string; // Adicionando as propriedades necessárias
  servico: string;
  cliente: string;
  email: string;
  photo?: string; // Coluna para a foto do usuário, opcional
}
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const agendamentos = await query<Empresa[]>(`
      SELECT a.id, a.data_hora, a.servico, u.name as cliente, u.contact as email, u.profile_picture as photo
      FROM agendamentos a
