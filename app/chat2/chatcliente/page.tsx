  ...
  useEffect(() => {
    const fetchAgendamentos = async () => {
      try {
        ...
        const response = await fetch("/api/conversas2", {
          method: "GET",
          headers: {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    if (newMessage.trim() && user && selectedAgendamento) {
      const message: Message = {
        ...
        agendamento_id: selectedAgendamento.id, // Usa o agendamento correto
        user_id: user.id,
        mensagem: newMessage,
          ...
          {filteredAgendamentos.length === 0 ? (
            ...
            filteredAgendamentos.map((agendamento) => (
              ...
              >
                <Avatar>
                  <AvatarImage
                    src={agendamento.photo || "/placeholder.svg?height=40&width=40"}
      ...
      <div className="flex-1 flex flex-col">
        <header className="bg-white p-4 flex items-center justify-between border-b">
          <div className="flex items-center">
            <h2 className="text-xl font-semibold">{user?.nome || "Selecione um Cliente"}</h2>
          </div>
        </header>
